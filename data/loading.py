...
def load(mode, data, num_train_human, num_test_human, num_train_synthetic, num_test_synthetic):
    if data == 'human':
        ...
        train = build_turk_observations_unified(turk_path, annotation_path, num_train_human, states, test_maps, test = False)
        ...
        test, stop_point = build_reinforce_observations_unified(turk_path, num_test_synthetic, states, stop_point)
        ...
        human_train = build_turk_observations_unified(turk_path, annotation_path, num_train_human, states, test_maps, test = False)
        human_test = build_turk_observations_unified(turk_path, annotation_path, num_test_human, states, test_maps, test = True)
        ...
        lower_bound = 0
        syn_train, stop_point = build_reinforce_observations_unified(turk_path, num_train_synthetic, states, lower_bound)
        syn_test, stop_point = build_reinforce_observations_unified(turk_path, num_test_synthetic, states, stop_point)
        
        print("Loaded {} synthetic examples...".format(syn_train[0].shape[0] + syn_test[0].shape[0]))
        
        train = []
        test = []
        for dim in range(len(human_train)):           
            if isinstance(human_train[dim], list):
                train.append(human_train[dim] + syn_train[dim]) 
            else: 
                train.append(np.concatenate((human_train[dim], syn_train[dim])))

        for dim in range(len(human_test)):           
            if isinstance(human_test[dim], list):
                test.append(human_test[dim] + syn_test[dim]) 
            else: 
...
def get_embedding(tokenizer, instructions):
    max_seq_length = 0
    embeddings = []
    pad_token = 0
    final_embeddings = []
    ...
    for tokens in embeddings:
        ...
        final_embeddings.append(tens)

    return final_embeddings


def get_bert_embeddings(model, instructions, type):  
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    # elif model == 'gpt2' or model == 'gpt2-large': 
    #     tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

    to_encode = []
    embeddings = get_embedding(tokenizer, instructions)
   
    return embeddings

   
def to_tensor_bert(model, data, text_vocab, data_type, embedding_type):
    layouts, objects, rewards, terminal, instructions, values, goals = data
   
    indices = get_bert_embeddings(model, instructions, data_type)
    indices = cudit(torch.stack(indices))
...
def initialize_states(data_path):
    if 'DATA_DIR' in os.environ:
        path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', data_path) 
        path = os.path.join(path, '0.p')
    else: 
...
def build_state_observations(data_path, worlds_list, states, states_dict, state_channels = 2):
    ...
    for ind, world_num in enumerate(worlds_list):
        path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', data_path)
        path = os.path.join(path, str(world_num) + '.p')
        info = pickle.load( open(path, 'r') )
        world = info['map']
...
def build_single(path, state_channels, goal_channels):
    if 'DATA_DIR' in os.environ: 
        path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', path) 
    info = pickle.load( open(path, 'r') )
    world = info['map']
    goals = info['goals']
...
def build_reinforce_observations_unified(data_path, num_required, states, lower_bound, state_channels = 1, goal_channels = 1, verbose = True):
    num_states = len(states)
    state_size = int(math.sqrt(num_states))
    ...
    last_seen_world = 0 
    if verbose:
        progress = tqdm(total = num_required)
    for world_ind, world_num in enumerate(range(lower_bound, lower_bound + num_required)):
        if verbose:
            progress.update(1)
        if 'DATA_DIR' in os.environ: 
            path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', data_path)
        ...
        num_needed = num_required - count 
        res = build_reinforce_single(path, state_channels, goal_channels, num_needed)
        if res is not None: 
            state_single, object_single, reward_single, terminal_single, \
                instruct_single, value_single, goals_single = res 
            state_obs = np.vstack( (state_obs, state_single) )
            object_obs = np.vstack( (object_obs, object_single) )
            ...
            goals_obs.extend( goals_single )
            # import ipdb; ipdb.set_trace() 
            count += state_single.shape[0]
            last_seen_world = world_num 
            if count >= num_required: 
                break 
    # pdb.set_trace()
    return (state_obs, object_obs, reward_obs, terminal_obs, instruct_obs, value_obs, goals_obs), last_seen_world


def build_reinforce_single(path, state_channels, goal_channels, num_needed):
    if 'DATA_DIR' in os.environ: 
        path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', path) 
    ...
    if num_needed < len(goals): 
        num_goals = num_needed
    else: 
        num_goals = len(goals)
    states = [(i,j) for i in range(world.shape[0]) for j in range(world.shape[1])]
    num_states = len(states)
    ...
    state_obs = np.zeros( (num_goals, state_channels, state_size, state_size) )
    object_obs = np.zeros( (num_goals, goal_channels, state_size, state_size) )
    reward_obs = np.zeros( (num_goals, 1, state_size, state_size) )
    terminal_obs = np.zeros( (num_goals, 1, state_size, state_size) )

    instruct_obs = []
    goal_obs = []

    value_obs = np.zeros( (num_goals, state_size, state_size) )

    objects = reconstruct_goal(world)
    
    count = 0
    for goal_ind, goal in enumerate(goals):
        # val_map = values[goal_ind]
        val_map = values[goal_ind]
        # instr = instructions[goal_ind]
        ...
        state = reconstruct_state(world, (0,0))[0]
        # for pos_ind, position in enumerate(states):
        object_obs[count] = objects
        state_obs[count] = state
...
def build_turk_observations_unified(data_path, annotation_path, num_worlds, states, test_maps, test = False, state_channels = 1, goal_channels = 1, verbose = True):
    ...
    for key in keys:
        ...
        if 'DATA_DIR' in os.environ: 
            data_path = os.path.join(os.environ['DATA_DIR'], 'spatial-reasoning', data_path) 
...
# def load_for_inference(mode, data, num):
#     turk_path = os.path.join('data', mode)
#     states, _ = initialize_states(turk_path)
...
#         print '\n<Data> Loading {} train environments with human annotations'.format(mode)
#         train = build_turk_observations_unified(turk_path, annotation_path, num, states, test_maps, test = False)
...
#     elif data == 'synthetic':
#         # raise RuntimeError('synthetic data is not cleaned up yet -- check back in a few days!')
#         train = build_reinforce_observations_unified(turk_path, range(num), states)
#         # pdb.set_trace()
...
# def get_statistics_inference(data):
#     ## unpack data
#     train_layouts,  train_objects,  train_rewards,  train_terminal, train_instructions, train_values,   train_goals = data

#     ## get word indices for all words in instructions
#     text_vocab = get_word_indices(train_instructions)
...
# def get_embedding(model, tokenizer, instructions): 
#     embeddings = []
#     for insn in instructions: 
#         input_ids = torch.tensor([tokenizer.encode("Here is some text to encode")])
#         last_hidden_states = model(input_ids)[0][0] # Models outputs are now tuples
#         embedding = torch.reshape(last_hidden_states, (5376,))
#         embeddings.append(embedding)
#     return embeddings 


# def get_bert_embeddings(instructions, type, model_path): 
#     # pdb.set_trace()
...
#                 embeddings.append(embed_dict[insn])
#             else:
#                 embed = get_embedding(model, tokenizer, [insn])[0]
#                 embeddings.append(embed)
#                 embed_dict[insn] = embed
#         pickle.dump( embed_dict, open( embeddings_file_name , "wb" ) )
#         return embeddings
#     else:
#         embed_dict = dict()
#         embeddings = get_embedding(model, tokenizer, instructions)
#         for i in range(len(embeddings)):
#             embed_dict[instructions[i]] = embeddings[i]
#         pickle.dump( embed_dict, open( embeddings_file_name , "wb" ) )
#         return embeddings
...
# def to_tensor_bert(data, text_vocab, data_type, model_path):
#     layouts, objects, rewards, terminal, instructions, values, goals = data
#     # full_instructions = copy.deepcopy(train_instructions + val_instructions)
...
#     # text_vocab_size = len(text_vocab) + 1
#     # indices = instructions_to_indices(instructions, text_vocab)
#     indices = get_bert_embeddings(instructions, data_type, model_path)
